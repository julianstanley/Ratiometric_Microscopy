...
roGFP1_R12_sensor <- new("redoxSensor",
                           newSensorFromSpectra(
                           spectraMatrixFromValues(roGFP1_R12_data$lambda_min[[1]],
                                                   roGFP1_R12_data$values_min[[1]],
                                                   roGFP1_R12_data$lambda_max[[1]],
                                                   roGFP1_R12_data$values_max[[1]]),
                           lambda_1 = c(395, 425), 
                           lambda_2 = c(460, 480)),
                           e0 = roGFP1_R12_data$sensor_midpoint[[1]])

error_model_generator <- function(abs_error) {
  return(function(x) {return(abs_error)})
}

plotdf <- data.frame()
sensor <- roGFP1_R12_sensor
errors <- c(0.01, 0.02, 0.027, 0.029, 0.03)
errors <- c(0.027,0.029, 0.05, 0.1, 0.2)
for (error in errors) {
  R <- getR(sensor)
